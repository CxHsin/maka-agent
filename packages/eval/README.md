...
The checked-in Terminal-Bench 2.1 six-arm cohort is `experiments/terminal-bench-2.1-deepseek-v4-flash-six-arm.json`; the four-arm file remains the compatibility subset. The six arms are Maka, Codex, Claude Code, Reasonix, OpenCode and Kimi Code. The spec freezes provider endpoints, highest native reasoning effort, framework version, container paths, read-only mounts and a 72-cell peak (`12 task groups × 6 arms`). External wrappers verify the pinned toolchain manifest and file checksums before model admission, meter provider traffic through a cell-local proxy, and retain trajectory and stderr artifacts under `/logs/agent`.

The optional seven-arm cohort is `experiments/terminal-bench-2.1-deepseek-v4-flash-seven-arm.json`. It adds the official ZCode headless CLI extracted from the pinned ZCode 3.7.5 Linux package (`zcode` CLI 0.16.1), keeps DeepSeek V4 Flash at `max`, and uses `10 task groups × 7 arms = 70 cells`. ZCode model-I/O and runtime JSONL are retained as separate trial artifacts.

Set each declared machine-path environment variable to the matching prepared toolchain directory and set the declared API-key credentials. Missing or mismatched machine paths fail before model admission and remain replaceable infrastructure attempts. The deployment environment is intentionally not part of the repository: host package installation and path selection are operator configuration, while profile behavior and expected toolchain identities are Eval semantics.
