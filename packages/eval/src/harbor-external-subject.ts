...
type Profile = 'codex' | 'claude-code' | 'pi' | 'reasonix' | 'opencode' | 'kimi-code' | 'zcode';
...
const PROFILE_IDENTITIES: Readonly<Record<Profile, ProfileIdentity>> = {
  codex: {
  ...
  pi: {
    ...
    version: '0.73.1',
    fingerprint: 'sha256:7bf5d492670c04fd7c599dee7e6eaabff964084affd216766107e6741df7a2e1',
    source: 'https://github.com/badlogic/pi-mono',
    sourceRevision: '781152fc24841dc54b22284514604048ebe5e2c9',
    license: 'MIT',
  },
  reasonix: {
...
try {
  ...
  const identity = await verifyToolchain(profile, systemRoot, command);
...
async function prepareProfile(
  ...
  if (selected === 'codex') {
    ...
    env.PI_OFFLINE = '1';
    env.PI_TELEMETRY = '0';
    const configRoot = join(home, '.pi', 'agent');
            ...
            models: [
              {
                ...
                maxTokens: 131_072,
                cost: { input: 0.145, output: 0.29, cacheRead: 0.0029, cacheWrite: 0 },
...
async function verifyToolchain(
  ...
  root: string,
  executable: string,
): Promise<{ version: string; fingerprint: string }> {
  const expected = PROFILE_IDENTITIES[selected];
  const expectedRoot = rooted(root, expected.root);
  if (!resolve(executable).startsWith(`${resolve(expectedRoot)}/`)) {
    return { version: 'test-command', fingerprint: 'unverified-test-command' };
  }
  const manifest = JSON.parse(await readFile(join(expectedRoot, 'manifest.json'), 'utf8')) as {
    fingerprint?: unknown;
  };
  if (manifest.fingerprint !== expected.fingerprint) {
    throw new Error(`${selected} toolchain fingerprint mismatch`);
  }
  const checksums = await readFile(join(expectedRoot, 'checksums.sha256'), 'utf8');
  for (const line of checksums.split('\n')) {
    if (!line.trim()) continue;
    const match = /^([a-f0-9]{64})\s+(.+)$/u.exec(line);
    if (!match) throw new Error(`${selected} toolchain checksum manifest is invalid`);
    const path = resolve(expectedRoot, match[2]);
    if (!path.startsWith(`${resolve(expectedRoot)}/`)) {
      throw new Error(`${selected} toolchain checksum path escaped`);
    }
    const digest = await hashFile(path);
    if (digest !== match[1]) throw new Error(`${selected} toolchain checksum mismatch`);
  }
...
function classifyExecution(
  ...
  if (requests > 0 || completed || reportedError) {
...
async function startMeteringProxy(
  ...
  const server = createServer((request, response) => {
    const operation = (async () => {
      ...
      response.end();
      const current = parser.finish();
      if (current) {
        measured = true;
        addUsage(total, current);
      }
    })().catch((error: unknown) => {
