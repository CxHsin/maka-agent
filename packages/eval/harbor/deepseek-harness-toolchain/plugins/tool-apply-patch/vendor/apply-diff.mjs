// Vendored from openai/openai-agents-js — the reference V4A diff applier.
//
// Source:  packages/agents-core/src/utils/applyDiff.ts
//          https://github.com/openai/openai-agents-js
//          commit e2cbca883dfccae90791c1e9237c15ef897a85d2
// License: MIT, Copyright (c) 2025 OpenAI (see ../../NOTICE)
//
// Vendored rather than depended on: `@openai/agents-core` unpacks to 9 MB and
// pulls the whole `openai` SDK into the dependency tree, and this arm's
// toolchain is fingerprinted file by file. This function has no imports at
// all, so vendoring it costs one file and adds no transitive surface.
//
// Below this header the file is the `tsc --target esnext --module esnext` emit
// of that source, unedited: the type annotations are gone and nothing else is.
// The upstream test suite travels with it in apply-diff.test.mjs, which is what
// establishes that the conversion changed no behaviour. Biome's formatter is
// configured to skip this directory so the emit stays byte-stable; do not
// hand-edit it. Carrying a fix means re-vendoring from a newer upstream commit.
