...
export interface ConversationCopy {
  ...
  composer: {
    ...
    queuedForCurrentTurn: string;
    queuedForNextTurn: string;
    retractQueueLabel: string;
    streaming: string;
    processing: string;
    continuing: string;
  ...
  messages: {
    ...
    /** Mid-turn steering injection (#1954): compact badge above the steered text. */
    steerBadge: string;
  };
  chat: {
    memory: string;
...
const CONVERSATION_COPY = {
  zh: {
    ...
    composer: {
      ...
      queueNextLabel: '下一轮发送', queuedForCurrentTurn: '等待加入当前回答', queuedForNextTurn: '下一轮', retractQueueLabel: '撤回到输入框',
    ...
    messages: {
      ...
      thinkingTruncatedTitle: '部分 reasoning 已截断；显示的是最近的内容', outputTruncatedTitle: '助手输出已超过单次回合上限，超出部分未渲染。如需完整内容请重新生成或查看持久化的会话日志。', removeAttachmentAriaLabel: (name) => `移除 ${name}`, quoteLabel: '引用', quoteExpandAriaLabel: '展开引用全文', quoteCollapseAriaLabel: '收起引用', removeQuoteAriaLabel: '移除引用', aborted: '(已中断)', abortedByStop: '(已中断 · 由停止按钮触发)', steerBadge: '引导已注入',
    },
    chat: {
  ...
  en: {
    ...
    composer: {
      ...
      queueNextLabel: 'Send next', queuedForCurrentTurn: 'Waiting for current response', queuedForNextTurn: 'Next turn', retractQueueLabel: 'Return to composer',
