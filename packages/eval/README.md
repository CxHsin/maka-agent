...
The harness ships no patch tool, so `harbor/deepseek-harness-toolchain/plugins/tool-apply-patch/` is this repository's, composed exactly as the other two editors are and reaching the filesystem through the same `ctx.fs`. Its grammar and model-facing description are Codex's, and applying one file's hunks is the OpenAI Agents SDK's `applyDiff`, vendored under `vendor/` rather than depended on because `@openai/agents-core` unpacks to 9 MB and pulls the whole `openai` SDK into a fingerprinted toolchain; that function has no imports, so vendoring it costs one file, and the upstream test suite travels with it as the evidence that the mechanical TypeScript-to-JavaScript conversion changed no behaviour. Both sources and their licences are recorded in `harbor/deepseek-harness-toolchain/plugins/NOTICE`. The tool deviates from Codex in two ways, both stated to the model in its own description rather than discovered by failure. It has no `*** Delete File:` and no `*** Move to:`, because `ctx.fs` exposes no delete or rename primitive and neither does any other harness file tool — in every arm the model removes and renames through bash, and reaching around that seam would put a second filesystem authority in the one arm being measured. And a patch either applies completely or changes nothing, because the other two contracts touch one file per call and cannot half-apply; letting this one leave a partly-patched tree would make the arms differ in failure modes rather than in edit format.
